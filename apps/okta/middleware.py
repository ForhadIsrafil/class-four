...
class OktaMiddleware:
    ...
    def __call__(self, request):
        ...
        skipped_path = ['api/v1/media/file', '/callback', '/get-access-token', '/signup', '/signin',
                        '/active-account', '/forget-password', '/reset-password', '/check-familyname']
        ...
        if req_user_info:
            ...
            if response:
                ...
                if okta_user == None:
                    ...
                    # create user in Messages_API
                    url = settings.MESSAGE_API_URL + '/number-list'  # Random Message API URL.....
                    headers = {
                        'Authorization': 'Bearer ' + session_id
                    }
                    try:
                        msg_req_res = requests.get(url, data={}, headers=headers)
                        if msg_req_res.status_code != 200:
                            return HttpResponse(json.dumps(msg_req_res.content), status=400)
                    except:
                        return HttpResponse(json.dumps({'error': 'Message API server response failed.'}), status=400)
